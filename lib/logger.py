...
class HTTPLog:
	...
	def __init__(self, log_level, log_name, log_format):
		if not config["logs"]["enabled"]:
			log(2,"Logs are disabled in _config.json")
			return
	...
	def create_output(self):
		if check_file_exists(self.log_file):
			log(2,"File %s already exists! It will be appended to." % self.log_file)

		if not create_dir_if_not_exists(self.log_dir):
			log(3,"Failed to create logs directory: %s" % self.log_dir)

		if not create_file_if_not_exists(self.log_file):
			log(3,"Failed to create log file: %s" % self.log_file)
		else:
			log(2,"Log file: %s" % self.log_file)

		if not check_can_write(self.log_file):
			log(3,"Failed to gain write permission for log file: %s" % self.log_file)
